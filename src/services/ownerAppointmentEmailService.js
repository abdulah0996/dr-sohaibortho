...
function buildOwnerAppointmentEmail({ appointment, clinicName, timezone }) {
  ...
  const values = {
    "Appointment ID": requiredText(appointment?.appointmentId),
    "Patient Name": patientName,
    "Contact Number": requiredText(appointment?.phoneE164),
...
async function sendOwnerAppointmentEmail({ appointment, outbox }) {
  ...
  const content = buildOwnerAppointmentEmail({
    appointment,
    clinicName: appointment.locationSnapshot?.clinicName || config.emailAppointmentAlert.fromName,
